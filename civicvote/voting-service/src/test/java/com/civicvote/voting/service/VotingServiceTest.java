...
@DisplayName("Voting Service — Core Business Logic & Anonymous Ballot Tests")
class VotingServiceTest {
    ...
    void castVote_ElectionNotActive_ThrowsException() {
        ...
        assertThrows(ElectionNotActiveException.class, () ->
                votingService.castVote(voteRequest, VOTER_REF));
    ...
    @Test
    @DisplayName("Duplicate vote rejected when voter already voted in this election")
    void castVote_DuplicateVote_ThrowsException() {
        ...
        when(voteRepository.existsByVoterReferenceAndElectionId(VOTER_REF, ELECTION_ID))
                .thenReturn(true);
    ...
    @Test
    @DisplayName("Valid vote is recorded and forwarded anonymously (no voter identity) to Result Service")
    void castVote_Success_AnonymousForwarding() {
        ...
        Vote savedVote = new Vote(VOTER_REF, ELECTION_ID, CANDIDATE_ID);
        when(voteRepository.save(any(Vote.class))).thenReturn(savedVote);

        VoteResponse response = votingService.castVote(voteRequest, VOTER_REF);
        ...
        // Verify vote was saved with voter reference locally
    ...
    @Test
    @DisplayName("hasVoted correctly queries repository")
    void hasVoted_ReturnsRepositoryStatus() {
        when(voteRepository.existsByVoterReferenceAndElectionId(VOTER_REF, ELECTION_ID))
                .thenReturn(true);

        assertTrue(votingService.hasVoted(VOTER_REF, ELECTION_ID));
        verify(voteRepository).existsByVoterReferenceAndElectionId(VOTER_REF, ELECTION_ID);
    }
}
